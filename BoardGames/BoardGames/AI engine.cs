...
namespace BoardGames
{
    public class AI_engine
    {
        ...
        /*
                int[,] pesakPozB = new int[,] { {929, 929, 929, 929, 929, 929, 929, 929 },
                ...
                int[,] skakacPozB = new int[,] {{-50,-40,-30,-30,-30,-30,-40,-50 },
                                                { -40,-20,  0,  0,  0,  0,-20,-40 },
                                                { -30,  0, 9, 15, 15, 9,  0,-30},
                ...
                int[,] skakacPozC = new int[,] {{-50,-0,-10,-30,-30,-30,-10,-50 },
                                                { -40,-20,  0,  5,  5,  0,-20,-40},
                                                {  -30,  5, 10, 15, 15, 10,  5,-30},
                                                { -30,  0, 15, 20, 20, 15,  0,-30},
                                                { -30,  5, 15, 20, 20, 15,  5,-30},
                ...
                int[,] lovacPozB = new int[,] {{-20,-10,-10,-10,-10,-10,-10,-20 },
                                                { -10,  0,  0,  0,  0,  0,  0,-10 },
                                                { -10,  0,  5, 10, 10,  5,  0,-10},
                                                { -10,  5,  5, 10, 10,  5,  5,-10},
                ...
                int[,] lovacPozC = new int[,] {{-20,-10,-10,-10,-10,-10,-10,-20 },
                                                { -10,  5,  0,  0,  0,  0,  5,-10 },
                                                { -10, 10, 10, 10, 10, 10, 10,-10},
                                                {-10,  0, 10, 10, 10, 10,  0,-10},
                                                { -10,  5,  5, 10, 10,  5,  5,-10},
                ...
                int[,] topPozB = new int[,] {{0,  0,  0,  0,  0,  0,  0,  0 },
                ...
                int[,] topPozC = new int[,] {{ 0,  0,  0,  5,  5,  0,  0,  0 },
                                            { -5,  0,  0,  0,  0,  0,  0, -5 },
                                            { -5,  0,  0,  0,  0,  0,  0, -5},
                                            { -5,  0,  0,  0,  0,  0,  0, -5},
                                            { -5,  0,  0,  0,  0,  0,  0, -5},
                ...
                int[,] damaPozB = new int[,] {{-20,-10,-10, -5, -5,-10,-10,-20 },
                                            { -10,  0,  0,  0,  0,  0,  0,-10 },
                                            { -10,  0,  5,  5,  5,  5,  0,-10},
                                             { -5,  0,  5,  5,  5,  5,  0, -5},
                                            { 0,  0,  5,  5,  5,  5,  0, -5},
                ...
                int[,] damaPozC = new int[,] {{-20,-10,-10, -5, -5,-10,-10,-20 },
                                            {  -10,  0,  5,  0,  0,  0,  0,-10 },
                                            { -10,  5,  5,  5,  5,  5,  0,-10},
                                             { 0,  0,  5,  5,  5,  5,  0, -5},
                                            { -5,  0,  5,  5,  5,  5,  0, -5},
                ...
                int[,] kraljPozB = new int[,] {{-30,-40,-40,-50,-50,-40,-40,-30 },
                ...
                int[,] kraljPozC = new int[,] {{20, 30, 10,  0,  0, 10, 30, 20 },
                                                { 20, 20,  0,  0,  0,  0, 20, 20 },
                                                {  -10,-20,-20,-20,-20,-20,-20,-10},
                                                { -20,-30,-30,-40,-40,-30,-30,-20},
                                                {  -30,-40,-40,-50,-50,-40,-40,-30},
        ...
        (long, Point, Point) GameMin(ref List<Figura> figure, ref PoljeInfo[,] polja, long alpha, int depth, long prethodnaVrednost)
        {
            if (depth == 0 || KrajIgre(ref figure, ref polja))
            {
                return (prethodnaVrednost, new Point(-1, -1), new Point(-1, -1));
            }

            long beta = long.MaxValue;
            Point best_move = new Point(-1, -1);
            Point figura = new Point(-1, -1);
            ...
            for (int h = 0; h < figure.Count; h++)
            {
                ...
                {
                    foreach (var potez in x.MoguciPotezi(figure, ref polja))
                    {
                        long skor = 0;
                        Point figpoz;
                        Point move;
                        x.OdigrajPotez(potez, ref figure, ref polja);
                        (skor, move, figpoz) = GameMax(ref figure, ref polja, beta, depth - 1);
                        if (skor <= beta)
                        {
                            beta = skor;
                            best_move = potez;
        ...
        public long StanjeTable(ref List<Figura> figure, ref PoljeInfo[,] polja)
        {
        ...
        long SkorZaFiguru(ref Figura figura)
        {
            long skor = 0;
            if (figura.GetBoja() == Boja.bela)
            {
        ...
        (long, Point, Point) GameMaxOptimizovano(ref List<Figura> figure, ref PoljeInfo[,] polja, long beta, int depth, long pocStanje)
        {
            if (depth == 0 || KrajIgre(ref figure, ref polja))
            {
                return (pocStanje, new Point(-1, -1), new Point(-1, -1));
            }

            long nigga = pocStanje;
            ...
            for (int h = 0; h < figure.Count; h++)
            {
                ...
                if (x.GetBoja() == Boja.bela)
                {
                    ...
                    foreach (var potez in komedija)
                    {
                        ...
                        Point xd;
                        Point cemuovo;
                        (skor, xd, cemuovo) = GameMinOptimizovano(ref kopija, ref kopolja, alpha, depth - 1, pocStanje);
                        pocStanje = nigga;
                        if (skor >= alpha)
                        {
                            alpha = skor;
        ...
        (long, Point, Point) GameMinOptimizovano(ref List<Figura> figure, ref PoljeInfo[,] polja, long alpha, int depth, long pocStanje)
        {
            if (depth == 0 || KrajIgre(ref figure, ref polja))
            {
                return (StanjeTable(ref figure, ref polja), new Point(-1, -1), new Point(-1, -1));
            ...
            for (int h = 0; h < figure.Count; h++)
            {
                ...
                if (x.GetBoja() == Boja.crna)
                {
                    ...
                    foreach (var potez in komedija)
                    {
                        ...
                        Point xd;
                        Point cemuovo;
                        (skor, xd, cemuovo) = GameMaxOptimizovano(ref kopija, ref kopolja, beta, depth - 1, pocStanje);
                        pocStanje = nigga;
                        if (skor <= beta)
                        {
                            beta = skor;
        ...
        public (long, Point, Point) GameOptimizovano(ref List<Figura> figure, ref PoljeInfo[,] polja, int depth, Boja boja, long pocStanje)
        {
            var kopija = KopiranjeListe(figure);
            var kopolja = KopiranjePolja(ref polja);
            if (boja == Boja.bela)
                return GameMaxOptimizovano(ref kopija, ref kopolja, long.MaxValue, depth, pocStanje);
            return GameMinOptimizovano(ref kopija, ref kopolja, long.MinValue, depth, pocStanje);
        }
